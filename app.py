...
st.markdown("### Explore how gaming habits impact student academic performance")
...
genre = st.sidebar.multiselect("Gaming Genre", df["gaming_genre"].unique(), df["gaming_genre"].unique())
stress = st.sidebar.multiselect("Stress Level", df["stress_level"].unique(), df["stress_level"].unique())
...
with col2:
    fig = px.histogram(
        ...
        x="gaming_hours",
        nbins=20,
        title="📊 Distribution of Gaming Hours"
    )
    st.plotly_chart(fig, use_container_width=True)
...
fig = px.scatter(
    ...
    y="study_hours",
    size="grades",
    color="addiction_score",
    title="Gaming vs Study vs Grades"
)
st.plotly_chart(fig, use_container_width=True)
...
# -----------------------------
st.markdown("---")
st.markdown("🚀 Built with Streamlit & Plotly | Clean • Interactive • Professional")
